...
@Component({
  selector: 'app-ver-producto-comprobante',
  templateUrl: './ver-producto-comprobante.component.html',
  styleUrls: ['./ver-producto-comprobante.component.scss']
})
export class VerProductoComprobanteComponent {
  @Input() ORDER_GESTIONADA_PRODUCTS:any = []
  ...
  onSubmit(){
    if (this.comprobanteForm.valid) {
      ...
      const orden = this.ORDER_GESTIONADA_PRODUCTS;
      const factura = {
        ...formValues,
        productos: orden,
  ...
  calcularTotales() {
    ...
    for (let p of this.ORDER_GESTIONADA_PRODUCTS) {
