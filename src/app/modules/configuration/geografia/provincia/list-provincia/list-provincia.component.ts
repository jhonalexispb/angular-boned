...
export class ListProvinciaComponent {
  search:string = '';
      ...
      listProvincia(page = 1){
        this.provinciaService.listProvincia(page,this.search).subscribe((resp: any) => {
      ...
      editProvincia(PROV:any){
        const modalRef = this.modalService.open(EditProvinciaComponent,{centered:true, size: 'md'})
      ...
      createProvincia(){
        const modalRef = this.modalService.open(CreateProvinciaComponent,{centered:true, size: 'md'})
        modalRef.componentInstance.DEPARTAMENTOS = this.DEPARTAMENTOS
      ...
      deleteProvincia(PROVINCIA:any){
        this.sweet.confirmar_borrado('¿Estás seguro?', `¿Deseas eliminar la provincia: ${PROVINCIA.name}?`).then((result:any) => {
            ...
            this.provinciaService.deleteProvincia(PROVINCIA.id).subscribe({
              next: (resp: any) => {
                if (resp.message === 403) {
                  ...
                  this.PROVINCIAS = this.PROVINCIAS.filter((b:any) => b.id !== PROVINCIA.id); // Eliminamos el rol de la lista
                  this.sweet.success('Eliminado', `la provincia ${PROVINCIA.name} ha sido eliminada correctamente`,'/assets/animations/general/borrado_exitoso.json');
                }
              },
              error: (error) => {
