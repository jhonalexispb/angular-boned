...
import { CreateEventoComponent } from '../../orden-compra/cronograma/create-evento/create-evento.component';
import { EditEventoComponent } from '../../orden-compra/cronograma/edit-evento/edit-evento.component';
import { CompraService } from '../../service/compra.service';

@Component({
  selector: 'app-edit-cronograma-order-compra',
  templateUrl: './edit-cronograma-order-compra.component.html',
  styleUrls: ['./edit-cronograma-order-compra.component.scss']
})
export class EditCronogramaOrderCompraComponent {
  constructor(
  ...
  private updateDailyTotal(event: any) {
    ...
    if (cell) {
      ...
      eventos.forEach((ev: any) => {
        if (ev.start === date) {
            totalAmount += parseFloat(ev.extendedProps?.saldo) || 0;
      ...
      if (totalAmount > 0) {
        ...
        totalContainer.innerHTML = `<b class="bg-info text-white p-2 responsive-text rounded d-none">S/${totalAmount.toFixed(2)}</b>`;
      }
    }
  }
  ...
  listCuotas(){
    ...
    this.compraService.obtenerCuotasParaEditarOrdenCompra(this.ID_COMPRA).subscribe((resp: any) => {
      this.cuotas = resp.cuotas_orden_compra.length

      const formGuardado:any = localStorage.getItem('compra_edit_selected');
      ...
      const eventoGuardado = localStorage.getItem('eventos_edit_compra_cuotas');
      if (eventoGuardado) {
        ...
        const total = eventos.reduce((acc:any, evento:any) => acc + (evento.extendedProps?.saldo || 0), 0);
        ...
        this.eventosPendientes = resp.cuotas_orden_compra.map((evento: any) => ({
          ...evento,
          extendedProps: {
              ...evento.extendedProps,
              saldo: parseFloat(evento.extendedProps?.saldo) || 0, // Convertir a float y manejar valores nulos
          },
          editable: true
        }));
        ...
        this.calendarOptions = { 
          ...this.calendarOptions, 
          events: [...(Array.isArray(this.calendarOptions.events) ? this.calendarOptions.events : []), ...this.eventosPendientes]
        };

        localStorage.setItem('eventos_edit_compra_cuotas', JSON.stringify(this.eventosPendientes));

        this.eventosPendientes.sort((a, b) => {
          return new Date(a.start).getTime() - new Date(b.start).getTime();
        });

        const total = this.eventosPendientes.reduce((acc:any, evento:any) => acc + (evento.extendedProps?.saldo || 0), 0);
        const totalMontoFixed = parseFloat(this.totalMonto.toFixed(2));
        const totalFixed = parseFloat(total.toFixed(2));
  ...
  openAddEventModal(date: string) {
    ...
    modalRef.componentInstance.LOCAL_STG = 'eventos_edit_compra_cuotas'
  ...
  addEventToCalendar(newEvent: any) {
    ...
    if (this.calendarComponent) {
      this.cuotas++
      const eventoGuardado:any = localStorage.getItem('eventos_edit_compra_cuotas');
      this.eventosPendientes = JSON.parse(eventoGuardado);
      this.eventosPendientes.sort((a, b) => {
        return new Date(a.start).getTime() - new Date(b.start).getTime();
      });
  ...
  updateEvent(event: any) {
    let eventosGuardados = JSON.parse(localStorage.getItem('eventos_edit_compra_cuotas') || '[]');
    ...
    if (index !== -1) {
      ...
      localStorage.setItem('eventos_edit_compra_cuotas', JSON.stringify(eventosGuardados));
      const eventoGuardado:any = localStorage.getItem('eventos_edit_compra_cuotas');
      this.eventosPendientes = JSON.parse(eventoGuardado);
      this.eventosPendientes.sort((a, b) => {
        return new Date(a.start).getTime() - new Date(b.start).getTime();
  ...
  borrar_evento(cuota:any){
    this.sweet.confirmar_borrado('Ups',`Esta seguro de borrar la cuota de S/ ${cuota.extendedProps.saldo}`).then((result:any) => {
      if (result.isConfirmed) {
        ...
        if (eventoGuardado) {
          ...
          localStorage.setItem('eventos_edit_compra_cuotas', JSON.stringify(eventos));

          this.sweet.success('Bien','la cuota fue eliminada correctamente')
          const evento_list:any = localStorage.getItem('eventos_edit_compra_cuotas');
  ...
  onSubmit() {
    ...
    this.sweet.confirmar('¿Estas seguro?',`¿Desea registrar la edicion de la compra?`,'/assets/animations/general/ojitos.json','Si, hagamoslo','Cancelar').then((result:any) => {
      if (result.isConfirmed) {
        ...
        const eventosCompraCuotas = JSON.parse(localStorage.getItem("eventos_edit_compra_cuotas") || "[]");
  ...
  regresar_compra(){
    this.router.navigate([`/compras/edit/edit_compra/${this.ID_COMPRA}`]);
  }
}
